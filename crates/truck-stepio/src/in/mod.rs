...
impl TryFrom<&SurfaceAny> for Surface {
    ...
    fn try_from(x: &SurfaceAny) -> Result<Self, Self::Error> {
        use SurfaceAny::*;
        Ok(match x {
            ElementarySurface(x) => Self::ElementarySurface(Box::new(x.as_ref().try_into()?)),
...
impl TryFrom<&ElementarySurfaceAny> for ElementarySurface {
    type Error = StepConvertingError;
    #[inline(always)]
    fn try_from(value: &ElementarySurfaceAny) -> Result<Self, StepConvertingError> {
...
impl TryFrom<&ToroidalSurface> for alias::ToroidalSurface {
    type Error = StepConvertingError;
    #[inline(always)]
    fn try_from(
        ToroidalSurface {
    ...
    ) -> Result<Self, StepConvertingError> {
        if *major_radius <= TOLERANCE || *minor_radius <= TOLERANCE {
            return Err("ToroidalSurface radius too small".into());
        }
