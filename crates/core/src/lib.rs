...
impl SpaceThumbnailsRenderer {
    ...
    pub fn load_step_asset(&mut self, path: &Path) -> Option<&mut Self> {
        ...
        // Keep the output path prefix consistent so MTL matches
        let out_prefix = std::env::temp_dir().join(format!("{}", uuid::Uuid::new_v4()));
        let out_path = out_prefix.with_extension("obj");
        ...
        match status {
            Ok(s) if s.success() => {
                ...
                // Load from the file directly to ensure MTL is found by Assimp
                let result = self.load_asset_from_file_internal(&out_path);
                
                // Cleanup temp files
                // Delay cleanup of MTL if possible? 
                // Actually, Assimp copies everything into its own memory structures during from_file.
                // Once from_file returns Ok(asset), the files are no longer needed.
                ...
                // Cleanup the log file generated by the batch script
                let log_file = out_path.with_extension("obj.log");
                if log_file.exists() {
    ...
    /// Internal helper to load asset from file without format sniffing (already known)
    fn load_asset_from_file_internal(&mut self, filepath: &Path) -> Option<&mut Self> {
        log_debug(&format!("Loading model: {:?}", filepath));
        ...
        match asset_res {
            Ok(asset) => {
                unsafe {
                    ...
                    let mut camera = self.engine.get_camera_component(&self.camera_entity).unwrap();
                    camera.set_exposure_physical(16.0, 1.0 / 125.0, 100.0);
        
                    // For STEP/IGES conversions, always use orthographic
            ...
            Err(e) => {
                log_debug(&format!("Assimp failed to load converted file: {:?}", e));
                None
            }
        }
    }
    ...
    pub fn load_step_asset_truck(&mut self, filepath: impl AsRef<Path>) -> Option<&mut Self> {
        self.load_step_asset(filepath.as_ref())
    }
...
unsafe fn setup_camera_surround_view(camera: &mut Camera, aabb: &Aabb, viewport: &Viewport, is_orthographic: bool) {
    ...
    if is_orthographic {
        ...
        // Max dimension of the box extent
        let extent = aabb.extent();
        let max_dim = extent[0].max(extent[1]).max(extent[2]);
        let max_half_dim = max_dim / 2.0;
        
        // Add some padding
        let zoom = 2.5; // Multiplier for half_extent to get full height with padding
...
fn fit_into_unit_cube(bounds: &Aabb) -> Mat4f {
    ...
    if max_extent < 1e-6 {
        return Mat4f::scaling(Float3::new(1.0, 1.0, 1.0));
    }
